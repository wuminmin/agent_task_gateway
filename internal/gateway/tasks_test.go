...
func TestRequestDataTaskUsesCompleteCatalogBudget(t *testing.T) {
	...
	if result["budget_source"] != "catalog_profile" || result["budget_profile"] != "detail-manual-v5" {
		t.Fatalf("budget was not issued from the complete catalog profile: %#v", result)
	}
	if budget["exposure_profile_version"] != "taskgate-exposure-v5" || budget["max_outcome_facts"] != int64(5) ||
		budget["predicate_footprint"] == nil {
		t.Fatalf("default approval route did not select the V5 outcome profile: %#v", result["budget"])
	}
