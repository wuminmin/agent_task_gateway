...
## 十二、最终方案（作者 2026-08-16 03:30 批准）

### 12.0 先更正一处：那个「2.3 倍」不是实测值

上一轮模拟主编视角时我举例说「经过治理比直连慢 2.3 倍」——**那是我编的示例数字，用来说明一个好结果长什么样，不是任何测量结果**。真实的开销倍数**目前没有人知道**，因为 baseline 的 `direct` 对照臂一格都没跑过。**本方案的核心目的就是把这个数测出来。** 在测出之前，论文里不得出现任何具体倍数。

### 12.1 目标

24 小时内交出一版可投的论文，核心新增证据是**治理开销倍数 + 重放零计费**；
副产物是**实测出的单格实现速率**，据此给 9/9 一个有依据的日期，而不是猜的日期。

### 12.2 为什么是 S1+S2 这 20 格

- 主编视角里唯一真正缺的关键数字就在这里：`direct` vs `novel` 的开销倍数
- 期望行数最大 50,000（S1/SF10、S5/SF10），**不超过今天已实测跑通的 100k×16**
- 复用 `provsql_orders` / `provsql_lineitem`，两者**已在活体 Catalog 里**
- 执行入口已存在：`run-real-pilot.sh` 走的 `go run ./evaluation/cmd/v5-full -config …`，
  是 config 驱动的，不需要新写 launcher

### 12.3 阶段与实测时长

| 阶段 | 内容 | 机器时长 | 依据 |
|---|---|---|---|
| **P0** | artifact 6 格 × 3 独立部署 × 3 样本（+5 预热） | **0.71 h** | 实测外推 |
| **P1** | A1 三层规则落 `AGENTS.md` 与任务模板；归档改指 `/mnt/d/wsl-data` | 纯代码 | — |
| **P2** | **实现 baseline S1+S2 共 20 格，逐格计时** | **未知（关键路径）** | 从未实现过 |
| **P3** | 生成轮 26 格 × 1 次 × 1 部署 + 作者签署 | 0.18 h + 10 min | 可算 |
| **P4** | 正式测量 26 格 × 30 样本 × 3 部署 | **7.65 h** | 可算 |
| **P5** | 论文改写与容器化构建 | 数小时 | — |

### 12.4 时间表（现在约 03:30，截止约次日 03:00）

```
03:30 ─ 04:15  P0 后台开跑（0.71 h）＋ P1 并行改规则
03:30 ─ 11:30  P2 实现 20 格 ← 关键路径，唯一未知
11:30 ─ 12:00  P3 生成轮 + 作者签署（需作者 10 分钟）
12:00 ─ 19:40  P4 正式测量 7.65 h
19:40 ─ 23:40  P5 论文改写与构建
23:40 ─ 03:00  缓冲
```

### 12.5 降级路径（预先写死，不临场决定）

| 触发条件 | 动作 |
|---|---|
| P2 到 **12:00** 仍未完成 | 放弃 S1+S2，只交 P0 的 artifact 证据 + 三处散文改写 |
| P2 完成但 **19:40** 前跑不完 3 部署 | P4 降为 **1 部署**（1.99 h），论文如实写单部署 |
| 任一阶段出现 measurement 后失败 | 按停手判据停手、留证、上报，**不重试不缩范围** |

### 12.6 必须说清的口径

P4 的证据**仍是 pilot 类**。原因是硬门禁：`run-deployment.sh:240` 要求九个 capability
全为 true 才允许启动 campaign，而实现 20 格后 baseline 仍是 20/58，capability 仍为 false。
所以论文按 pilot 如实报告——**数字是真的，只是不声称跑过冻结的发表级 campaign**。

按模拟主编视角的判断：`9/9` 与 `82 格` 这些词不会出现在稿子里，
审稿人看的是开销倍数与重放零计费这两个结果本身。

### 12.7 需要作者的三个动作

1. **现在**：批准翻 `artifactRealSystemValidated`（源码口径已满足，翻完 7/9）
2. **约 8 小时后（P3）**：逐字节签署生成的期望摘要与 oracle manifest，红线 1 不可代签
3. **P5 期间**：审阅论文三处散文改写

### 12.8 本方案明确不承诺

- **不承诺 24 小时内达成 9/9**。理由见十一与本节：76 格实现工期未知，
  且原始 126 格 campaign 的测量下界就是 35.8 小时。
- **不承诺具体开销倍数**。测出多少报多少，难看也照报。
- 9/9 的真实日期，**待 P2 测出单格实现速率后再给**。

---

## 十三、36 小时版最终方案（作者 2026-08-16 03:45 放宽窗口，并要求把优化并进来）

作者两条要求：不要反复跑带库全量；把实验方案优化做进来。窗口放宽到 36 小时
（约至 2026-08-17 15:45）。本节取代第十二节的时间安排，其余内容仍适用。

### 13.1 带库全量：本次冲刺只跑两次，写死

按 A1 三层规则，冲刺期间**全量只在两个门禁点跑**：

1. **P2 实现完成后一次**——20 格新代码入库前的回归门禁
2. **交付前一次**——终稿提交前的最终门禁

其余一律只跑**受影响的包**加聚焦测试。以今天为例：两个 observer 提交的受影响包实测
`evaluation/cmd/final-v5-observer` 0.269 s + `evaluation/internal/experiment` 89.060 s，
合计 89 秒，而全量是 3630 秒——**40 倍差距**。

### 13.2 实验侧优化：把 provisioning 从每样本摊到每格（本节最有价值的一条）

**实查依据：**

- baseline 的 `setup` **没有** `fresh_root` 字段（只有 scale 有）：
  实际字段仅 `fresh_deployment`、`oa_and_task_provisioning`、`warmups`、`warmup_seed`、
  `publication_probe_required`
- `measured.include_provisioning: false`，测量窗口是
  `immediately_before_arm_query_call` 到 `after_complete_logical_result_drain`
- S1+S2 的 20 格按模式恰好是 **4 direct + 4 novel + 4 semantic_replay +
  4 idempotent_replay + 4 normalized_rewrite_replay**
- 4 个 `direct` 格的 `bdg.active: false`、`entrypoint: postgresql_sql`
  ——**纯 PostgreSQL，不经 BDG、不需要 OA 审批与 task**

**由此得出三类成本，实现时必须按类分别设计：**

| 类别 | 格数 | 每样本成本 | 设计 |
|---|---|---|---|
| `direct` | 4 | 仅 SQL 执行 | 30 样本 = 30 次纯 SQL，**不建 task、不走审批**，近乎免费 |
| 三种 replay | 12 | 一次 novel 前置 + 30 次重放 | provisioning 每格一次，**不是每样本一次** |
| `novel` | 4 | **不可摊薄** | 每样本必须是真新颖事件，否则第二次起就是零新颖 |

**这条把 P4 的成本从「20 格 × 35 × 9.67 s」的粗算显著拉低**，因为只有 4 个 `novel` 格
承担不可摊薄的 provisioning。精确值待 P2 实现后标定，**本节不给具体数字**。

**这不是走捷径**：契约明写 provisioning 不计入测量窗口，`direct` 臂明写不经 BDG。
按契约实现即得此结果。

### 13.3 A3 放在 P4 测量期间做（机器忙、人闲）

A3 的改法已由 A2 榜单坐实，不再是推断：`newV10Harness` → `installCatalogV4SnapshotRegistry`
遍历 catalog 全部 7 个 snapshot publication 逐个 `snapshotbundle.Compile`，
22 个测试各重编译一次，形成 106.8–110.5 s 的窄带；编译确定性由测试自身断言
（`harness_test.go:379` 比对 catalog 已 pin 的 digest）。

改法：按包编译一次共享只读。预期 3630 s → 约 1400 s。

**排期**：放在 P4 的测量小时里做，**不占关键路径**。验收仍是「PASS 测试名集合逐个一致，
只准变快不准变少」。若 A3 未按时完成或验收不过，**直接放弃，不影响交付**。

### 13.4 36 小时时间表（现在约 03:45，截止约次日 15:45）

```
03:45 ─ 04:30  P0 artifact 6 格 × 3 部署 × 3 样本（0.71 h，后台）
               P1 并行：A1 三层规则落地 + 归档改指 /mnt/d/wsl-data
03:45 ─ 13:45  P2 实现 baseline S1+S2 20 格（关键路径，按 13.2 分三类设计）
13:45 ─ 14:30  门禁全量第一次 + 生成轮 26 格 + 作者签署
14:30 ─ 22:00  P4 正式测量（含 A3 并行改造）
22:00 ─ 04:00  P5 论文改写与容器化构建
04:00 ─ 12:00  缓冲 + 若有余力加 S6 12 格
12:00 ─ 15:45  门禁全量第二次 + 终稿留证
```
...
| 触发 | 动作 |
|---|---|
| P2 到 **13:45** 未完成 | 放弃 S1+S2，只交 P0 证据 + 三处散文改写 |
| P4 到 **22:00** 跑不完 3 部署 | 降为 1 部署，论文如实写单部署 |
| A3 验收不过 | 直接放弃，不影响交付 |
| measurement 后失败 | 按停手判据停手、留证、上报，不重试不缩范围 |

### 13.6 多出的 12 小时用在哪

不用来扩大实验范围，而是用来**降低失败概率**：P2 从 8 小时放宽到 10 小时，
P5 从 4 小时放宽到 6 小时，末尾留 8 小时缓冲。**唯有在 04:00 时 P0–P5 全部完成**，
才考虑追加 S6 那 12 格（它复用今天已验证的 result-heavy 路径，是 118 格里最省的一批）。
