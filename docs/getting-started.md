...
将输出填入 `GATEWAY_DATA_KEY`，并替换 `.env` 中全部空密码、Token 和共享密钥。系统控制库与业务库使用不同的管理员密码和应用密码：
...
三个请求应返回 `204 No Content`。Gateway readiness 同时检查系统控制 PostgreSQL、业务 PostgreSQL，以及是否存在尚未成功持久化的预算结算。
...
| 业务 PostgreSQL | `127.0.0.1:25434` | 回环地址 |
...
| 身份 | 工具 |
|---|---|
| Alice | `list_data_products`、`request_data_task`、`list_my_tasks`、`get_task_status`、`wait_for_approval`、`get_task_context`、`execute_plan`、`query_sql`、`get_query_result`、`get_budget`、`list_receipts`、`complete_task` |
...
`request_data_task` 返回 `task_id`、`oa_url`、审批模式和授权快照。用 `.env` 的 `OA_ALICE_PASSWORD` 以 `alice` 登录 OA，打开草稿并提交。低敏 `expense_summary` 会自动批准。
...
- `list_receipts`：请求摘要、SQL 指纹、目录版本、预算前后和结果 SHA-256。
...
再次启动时，Gateway 从 `control-pg-data` 恢复任务、Grant、预算、加密结果与审计链；业务数据保存在独立的 `business-pg-data`。不确定是否完成的 RESERVED 查询按完整预留量保守计费并标记 `INTERRUPTED`，PROCESSING 回调变为可重试。OA Demo 草稿仍在内存中，OA 容器重启后会丢失。
...
| Navicat 连接失败 | 确认 Compose 已启动、使用 `127.0.0.1`、端口分别为 25433/25434，并核对对应 `.env` 密码 |
