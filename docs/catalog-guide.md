...
所有数值必须为正；`query_timeout` 不能大于 `max_db_time`。时间使用 Go duration 字符串，如 `500ms`、`5s`、`15m`。含义：
...
客户端 `requested_budget` 当前只接受 `max_queries` 和 `max_rows`，且只能缩小 Profile 上限。单次查询会先预留预算，实际结算后释放未使用部分；达到任一硬上限自动归档任务。
...
3. 更新 Catalog，确认逻辑字段、类型、Scope 字段和 View 一致。
