...
### (c3) 语料口径定案（2026-09-01，规则 8 对齐；撤回上一节的 v2 重推计划）

语料 v1 的逐句 dependency/release 数按**保守先验上界**语义成立（实体键+scope+全引用列的 rows×fields 上界；
不含 LIMIT/HAVING 截断），预算由其在任何执行之前公式化导出——这正是第 8 条做法要求的保守先验，
**不得**用实测值重拟合预算（上一节的 v2 重推若用于改预算即违反该条，撤回）。实测计费（novel 增量、
HAVING/LIMIT 截断、聚合见证细则）是**测量结果**，进报告与补充材料，与先验上界并列呈现；
