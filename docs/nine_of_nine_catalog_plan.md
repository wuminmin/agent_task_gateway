...
> **2026-08-16 当日作废（第四节与第六节）。** 本文第四节提出的"给 Baseline S1/S2/S5
> 新建专属 Product"方案**没有执行，也不应执行**；第六节请作者裁决的那项契约修订
> 虽已获批准，但随后查明**不必要**，已如实告知作者并未执行。
>
> **推翻它的依据**：已评审的 Catalog 候选 `evaluation/final-v5-wsl2/catalog/benchmark-contract-v1.yaml`
> 早已给出另一套方案——不新建 Product，而是让全部六个 low 类基准 Product 共用一条更宽的
> 默认 low 路由；`finalv5contracts.validateCatalogTaskPolicies` 就是这么断言的。按 CLAUDE.md
> 第 6 条，已评审契约候选的权威性高于我在本文里的推导。**实际落地见提交 `f2c38fb`**，
> 其中三处偏离候选的取值各有实测依据（行上限 400000、不给 expense_summary 加独占路由、
> 谓词足迹与 summary 路由一致）。
>
> **第一、二、三、五节仍然有效**：一次性改完的理由、各家族的实查需求、
> 精确集合匹配加路由排他这两条源码级约束、以及"先攒后测只付一次重测代价"的执行顺序，
> 都未被推翻。第四节请只当作一条被否决的备选记录来读。
