...
Both private directories must be mode `0700`; every config and deployment binding must be mode `0600`. The three deployment binding files must be byte-for-byte identical. A binding has exactly the three top-level keys `dataset_sha256`, `catalog_sha256`, and `final_v5_adapter_v1`; the strict adapter section contains exactly its schema version plus scale, artifact, and ProvSQL bindings. It must provide the exact 12 dependency-scale cells, six artifact cells, and 105 ProvSQL nonce cells. Unknown or duplicate JSON keys, `deployment_volume_id_sha256`, private probe SQL, observer argv, executable paths, credentials, and extra cells are rejected.

Publication preflight parses every bound query as PostgreSQL SQL, permits one read-only `SELECT`, and proves its products, columns, scopes, observer relations, approval route, query/row/exposure budgets, and exact Catalog digest against source-controlled `config/catalog.yaml`. The dataset probe and observer are compiled from the frozen source; private JSON cannot select either execution path. Publication runs capture the live Gateway Catalog bytes, require observed dataset/Catalog digests to equal the reviewed binding, and derive the deployment-volume identity from the actual Compose volume set plus both PostgreSQL system identifiers. The finalizers independently recheck these identities across all three deployments and all nine experiment directories. The current Catalog does not yet authorize the complete formal scale/artifact matrix, so this gate intentionally blocks; do not weaken it or insert synthetic placeholders. Copy the example configs, replace the campaign ID and zero commit, and keep credentials in environment variables—not in configs or bindings. Stage B changed the frozen protocol hash to bind the profile-specific replicate matrix; any private config carrying the earlier hash must be regenerated and reviewed rather than silently reused.
