...
func TestBenchmarkS2S4S5QueriesAndCanonicalNormalizerRemainFrozen(t *testing.T) {
	...
	historicalSHA256 := map[string]string{
		// Regenerated by contract release v1.5, which corrected the S5 allowed
		// claim to its measured extent and replaced decision 10's grammar
		...
		// Also regenerated by v1.5, together with the 135 oracle manifests that
		// pin its indexed digest as normalization_spec_sha256.
		"contracts/result-normalization-v1.json": "9e78abdc2130b19cb414d520edeb6da4dfac0f481609efd244f22f6ba2b1e816",
	}
	...
	// The v1.4 literal carried a grammar rationale ("production
	// Join/semantic-View/UNION grammar forbids ORDER BY"). v1.5 regenerated it
	// to decision 10's authoritative prose, which rests on the frozen queries
	// defining no total result order. Regenerating this string re-pins
	// normalization_spec_sha256 in all 135 oracle manifests, so the release
	// regenerated those against a live PostgreSQL in the same pass. The
	// ordering modes themselves are unchanged; only a future contract release
	// may regenerate this byte string again.
	const totalOrderRule = "S1, S3, S6, Artifact, and single-Product Scale results use query_order_v1; " +
		"S2, S4, and S5 use canonical_typed_row_lexicographic_v1 because those frozen queries define no total result order and therefore do not rely on returned row order"
