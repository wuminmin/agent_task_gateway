...
// footprintFor is the footprint Stage N4 measures for an E-entry
// ExpectedSchema: one internal key, E calls per Attestation, in every scope.
// Tests that need a footprint disagreeing with that build their own.
func footprintFor(t *testing.T, entries int64) AttestationFootprintV2 {
	t.Helper()
	return footprintWithScopeCalls(t, entries, map[AttestationScope]int64{
		AttestationScopeConstructorOrColdPool:  entries,
		AttestationScopeExplicitPreflightPool:  entries,
		AttestationScopeSingleQueryTransaction: entries,
		AttestationScopePairedQueryTransaction: entries,
	})
}
...
func TestPairedNovelPlanReproducesTheMeasuredSixteen(t *testing.T) {
	...
	for class, want := range map[GatewayStatementClassV3]int64{
		...
		V3PostgreSQLInternalAttestation: 2,
...
func TestSemanticReplayPlanExpectsFourNotZero(t *testing.T) {
	...
	for class, want := range map[GatewayStatementClassV3]int64{
		...
		V3ViewDefinitionAttest: 1, V3PostgreSQLInternalAttestation: 1,
	} {
		if expected[class] != want {
			t.Errorf("class %s = %d, want %d", class, expected[class], want)
		}
...
func TestPerEntryClassesFollowTheExpectedSchemaCount(t *testing.T) {
	for _, entries := range []int64{1, 2, 5} {
		...
		for _, class := range []GatewayStatementClassV3{
			V3ViewColumnAttestation, V3ViewDefinitionAttest, V3PostgreSQLInternalAttestation,
		} {
			if expected[class] != want {
...
func TestNestedCountComesFromTheFootprintNotFromE(t *testing.T) {
	doubled := footprintWithScopeCalls(t, 1, map[AttestationScope]int64{
		AttestationScopeConstructorOrColdPool:  2,
		AttestationScopeExplicitPreflightPool:  2,
		AttestationScopeSingleQueryTransaction: 2,
		AttestationScopePairedQueryTransaction: 2,
	})
	...
	expected := plan.Expected()
	if expected[V3PostgreSQLInternalAttestation] != 4 {
		t.Fatalf("nested lookups = %d, want the 4 the footprint measures", expected[V3PostgreSQLInternalAttestation])
	}
	...
	if expected[V3PostgreSQLInternalAttestation] == expected[V3ViewDefinitionAttest] {
...
func TestPlanMultipliesAttestationScopesSeparately(t *testing.T) {
	...
	if got := plan.Expected()[V3PostgreSQLInternalAttestation]; got != 5 {
...
func TestRequireFootprintRecomputesTheNestedCount(t *testing.T) {
	...
	plan := mustPairedNovel(t, 1)
	if err := plan.RequireFootprint(footprint, testRuntimeIdentity()); err != nil {
		t.Fatalf("the qualifying footprint must satisfy its own plan: %v", err)
	}

	tampered := plan
	tampered.InternalExpectation = []InternalExpectation{
		{StrictASTSHA256: testInternalKeyA, Calls: 3},
	}
	if err := tampered.RequireFootprint(footprint, testRuntimeIdentity()); err == nil {
	...
	elsewhere := footprintFor(t, 1)
	elsewhere.PostgreSQL.LocalImageID = "sha256:" + strings.Repeat("9", 64)
	elsewhere.PostgreSQL.ContainerImageID = elsewhere.PostgreSQL.LocalImageID
	if err := plan.RequireFootprint(elsewhere, testRuntimeIdentity()); err == nil {
		t.Fatal("a footprint qualified on another image satisfied the plan")
	}
	otherRun := testRuntimeIdentity()
	otherRun.Platform = "linux/arm64"
	if err := plan.RequireFootprint(footprint, otherRun); err == nil {
		t.Fatal("a run on another PostgreSQL platform was accepted")
	}
}
...
func TestRequireFootprintIsVacuousForIdempotentReplay(t *testing.T) {
	plan := mustIdempotentReplay(t)
	if err := plan.RequireFootprint(AttestationFootprintV2{}, testRuntimeIdentity()); err != nil {
...
func TestExpectedSchemaEntriesAndDigestArePresenceCoupled(t *testing.T) {
	...
	idempotent = mustIdempotentReplay(t)
	idempotent.InternalExpectation = []InternalExpectation{{StrictASTSHA256: testInternalKeyA, Calls: 1}}
	if err := idempotent.Validate(); err == nil {
		t.Fatal("a plan performing no Attestation was allowed to expect internal statements")
	}
}
