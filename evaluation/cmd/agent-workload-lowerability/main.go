...
func products() map[string]queryplan.Product {
	return map[string]queryplan.Product{
		"expense_detail": product("expense_detail", []string{"receipt_no"}, map[string]string{"receipt_no": "text", "employee_no": "text", "employee_name": "text", "department": "text", "expense_date": "date", "expense_type": "text", "amount": "numeric", "city": "text", "purpose": "text", "status": "text"}, "sum", "count", "min", "max"),
		"expense_summary": product("expense_summary", []string{"month", "department", "expense_type"}, map[string]string{"month": "text", "department": "text", "expense_type": "text", "total_amount": "numeric", "request_count": "bigint"}, "sum", "count", "min", "max"),
		"provsql_orders": product("provsql_orders", []string{"orderkey"}, map[string]string{"orderkey": "bigint", "status": "bigint", "partition_key": "integer"}, "sum", "count"),
		"provsql_lineitem": product("provsql_lineitem", []string{"orderkey", "linenumber"}, map[string]string{"orderkey": "bigint", "linenumber": "integer", "extendedprice": "numeric", "partition_key": "integer"}, "sum", "count"),
