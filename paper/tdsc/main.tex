...
\title{TaskGate: Task-Scoped, Policy-Approved Database Access\\
...
choice between a broad reusable credential and repeated operation-by-operation
approval.  We
...
digests, expiry, and cumulative query, result-row, and database-time budgets.
Catalog policy first bounds this request and chooses either automatic or manual
approval.  The automatic route issues the unchanged envelope; the manual route
may reject it or issue only a monotonic narrowing.  A separate Go
...
intent.  We give six separately checked finite TLA+ abstractions and an
action-to-implementation audit map; we do not claim a composed model or a
mechanized refinement proof.  The evaluation design covers policy bypass, fault
injection, and four performance baselines.  This artifact draft does not
...
\begin{IEEEkeywords}
agent authorization, database security, usage control, policy approval,
...
credential.  The agent requests logical data products, columns, mandatory scope
values, and a bounded lifetime.  Catalog policy rejects requests outside its
deployment-wide ceiling and selects an office-automation (OA) approval route.
The low-sensitivity demo route approves the unchanged request automatically; the
manual route displays the request to a designated approver, who may approve,
reject, or narrow it.
...
\begin{enumerate}
  ...
  \item \emph{What task entitlement may be activated?}  Catalog policy bounds a
  versioned, RFC~8785-canonicalized manifest; a narrowing-only grant defines the
  task envelope.  The gateway checks callback actors against the policy-selected
  route, while the OA-signed receipt binds the decision and object digests.
...
\begin{itemize}
  \item a catalog-bounded authorization object and partial order under which an
  automatic or manual OA decision can only preserve or remove products, columns,
  scope values, time, sensitivity, or budget;
  ...
  claims; and
  \item six finite design models, checked separately, plus an auditable mapping
  from modeled actions to implementation mechanisms and tests, and a
  source-backed evaluation contract that keeps absent measurements visible.
\end{itemize}
...
agent-authorization protocols.  Its value is their engineering integration and
the explicit failure semantics for a concrete read-only database path; this is
not a claim that the separate formal models compose into an end-to-end theorem.
...
RBAC assigns permissions through roles and constraints, while ABAC evaluates
subject, object, operation, and environmental attributes against policy
~\cite{ferraiolo2001rbac,hu2019abac}.  These models are natural sources of a
principal's baseline enterprise entitlement.  \system addresses a later and
shorter-lived decision: activating a task envelope within a catalog ceiling and
then accounting for its use.  The prototype does not ingest roles or subject
attributes; its catalog ceiling is shared by authenticated demo subjects.  A
production entitlement decision would therefore precede \system and be
intersected with the catalog ceiling before an approval draft is created.
...
Macaroons encode contextual caveats in attenuable bearer credentials and support
decentralized delegation~\cite{birgisson2014macaroons}.  \system shares the
monotonic-authority intuition, but its grant is neither a freely delegable
capability nor sufficient by possession alone: every operation also consults
online task, request, and budget state.  This distinction permits durable
cumulative accounting and conservative handling of ambiguous outcomes.

\subsection{Database Query Mediation}

Qapla stores row/column policies in SQL and specializes queries using identity,
time, and database values~\cite{mehta2017qapla}.  ShillDB uses table capabilities
and executable contracts to confine the database authority of application
components~\cite{zigmond2020shilldb}.  Blockaid intercepts web-application
queries and uses SMT reasoning to check compliance with view-based data-access
policies while preserving application semantics~\cite{zhang2022blockaid}.
These systems provide richer application-oriented policy mechanisms than
\system's deliberately closed language.  \system instead chooses a restricted
SQL subset and deterministically rewrites approved logical products to reporting
views with mandatory predicates and a result cap.  This sacrifices
compatibility with arbitrary applications in exchange for a smaller policy
language, and adds task grants, cumulative budgets, retry records, and
...
Cryptographic audit logging has long distinguished tamper evidence from the
stronger guarantees obtained through forward-secure key evolution or trusted
storage~\cite{schneier1999audit}.  \system's transactional hash chain and signed
checkpoints authenticate retained evidence relative to a trusted prefix.  They
do not provide forward integrity after signing-key compromise, prevent
privileged wholesale deletion, or supply the external WORM retention and trusted
timestamps needed for stronger audit claims.
...
RBAC/ABAC~\cite{ferraiolo2001rbac,hu2019abac} & Role/attribute-derived enterprise entitlement & Baseline authorization administration and decisions & Expected upstream source of principal eligibility; the prototype has only a deployment-wide catalog ceiling. \\
...
Macaroons~\cite{birgisson2014macaroons} & Attenuable credential with contextual caveats & Decentralized delegation and contextual confinement & Similar monotonic restriction; \system uses non-delegable grants plus online cumulative state. \\
Qapla, ShillDB, Blockaid~\cite{mehta2017qapla,zigmond2020shilldb,zhang2022blockaid} & SQL policy, database capability/contract, or view policy & Fine-grained application query mediation & Richer application policy; \system adds task approval, bounded usage, and crash semantics to a smaller SQL language. \\
...
``Policy-approved'' covers both catalog-selected routes, but the evidentiary
meaning differs.  On the manual route, OA records a decision attributed to the
configured authenticated approver.  On the explicitly labeled low-sensitivity
automatic route, submission by the authenticated requester causes OA to sign the
unchanged policy-admissible core with that requester as actor.  The latter is a
policy decision and requester assent, not review by a second human.  Any claim or
experiment specifically about human review must therefore select the manual
route and must not generalize to automatic approvals.

\section{Authorization Model}
\label{sec:model}

\subsection{Entitlement Ceiling, Manifest, and Grant}

We use \emph{entitlement} for the maximum authority that policy makes eligible
for activation, and \emph{grant} for the immutable, task-specific authority that
the gateway actually enforces.  For requested products $P$, catalog resolution
constructs a ceiling $E_{\mathrm{cat}}(P)$ from cataloged products and columns, allowed
scope domains, the highest effective sensitivity, the corresponding approval
route, and that route's budget profile.  Let $\mathsf{auth}(X)$ project an object
onto products, per-product columns, mandatory scopes, sensitivity, lifetime, and
budget.  Using the component-wise order defined below, the gateway creates a
manifest only after checking

\begin{equation}
  \mathsf{auth}(M)\preceq E_{\mathrm{cat}}(P).
  \label{eq:catalog-entitlement}
\end{equation}

This ceiling is deployment-wide in the prototype: authentication supplies stable
subject and agent identifiers, but no RBAC/ABAC or per-subject entitlement feed
is queried.  Consequently, $E_{\mathrm{cat}}$ must not be interpreted as proof that a
particular employee is organizationally entitled to a product.  A production
integration would additionally require an identity-policy ceiling $E_I(h,a)$ and
admit $M$ only below both $E_I(h,a)$ and $E_{\mathrm{cat}}$; that intersection is outside
the implemented and formally checked system.
...
Let $G_M$ be the unchanged core derived from $M$.  Catalog sensitivity selects a
route $r$.  The automatic demo route emits $G=G_M$ after requester submission;
the manual route permits reject, $G=G_M$, or $G\preceq G_M$.  Thus every
successful prototype decision satisfies

\begin{equation}
  \mathsf{auth}(G)\preceq\mathsf{auth}(M)\preceq E_{\mathrm{cat}}(P).
  \label{eq:approval-chain}
\end{equation}

Approval is therefore an activation and possible attenuation of already
catalog-admissible authority.  It cannot manufacture authority beyond the
catalog ceiling.  Conversely, because the prototype ceiling is not
principal-specific, the approval receipt is not evidence of an upstream
enterprise entitlement decision.
...
\begin{enumerate}
  ...
  fields, rejects any request outside $E_{\mathrm{cat}}(P)$, normalizes sets, creates
  $M$, and computes $H_M$.
...
The TLA+ suite~\cite{lamport2002specifying} contains six finite abstractions that
are model checked separately.  No submitted configuration forms their product,
and successful checks of the six configurations must not be read as a theorem
about their conjunction or about the Go/PostgreSQL implementation.
...
\subsection{Finite-State Checks and Local Safety Properties}

Within the constants and abstraction boundary of the model that defines it, TLC
checks each of the following named properties.  A property listed in one split
model is not silently imported into another:
...
and recovery models check their own finite boundaries.  Their results cannot be
conjoined into a system-level invariant without an explicit composition argument
covering shared variables and rely/guarantee assumptions; no such argument is
provided.  Nor do they prove that Go and PostgreSQL refine a single composed
TLA+ specification.

Three local preservation arguments explain the modeled transitions.  First, reservation
...
it, so duplicate attempts cannot execute it again.  These are arguments about
the abstractions, not mechanized proofs.  Connecting them to production behavior
depends on Control PG atomicity, all writers using the mapped transactions, and
the unproved assumptions at model boundaries.
...
\input{generated/formal_status.tex}
...
\input{generated/security_status.tex}
...
\input{generated/performance_status.tex}
...
\input{generated/approval_count.tex}
...
\input{generated/artifact_status.tex}
...
\midrule
Catalog-bounded manifest identity and digest binding; approval can only preserve or narrow & \code{internal/catalog}, \code{internal/domain/authorization.go}, \code{internal/approval/protocol.go}, OA callback validation & Unit vectors and narrowing/callback tests; no principal-specific enterprise entitlement source is implemented. \\
...
Composed end-to-end safety across SQL, vector budget, audit, receipts, and recovery & No claim in this revision; these concerns are split across finite models and linked to code only by \code{formal/REFINEMENT.md} & Separate TLC results and tests are evidence for their named boundaries, not a composition or mechanized refinement theorem. \\
...
\begin{itemize}
  ...
  \item \textbf{No principal-specific enterprise entitlement.}  The prototype
  authenticates subjects but applies one deployment-wide Catalog to them; it
  does not evaluate organizational roles or subject attributes.  Manual approval
  can attenuate a request but is not a substitute for an upstream RBAC/ABAC
  eligibility decision.  Production deployment must intersect that decision
...
schema attestation, and scoped receipts are implemented together to mitigate
authorization drift, retry, concurrency, and crash ambiguity in the fixed
single-gateway PostgreSQL deployment.  The separate model checks and tests do
not establish a composed whole-system safety theorem.  The mechanism is best
understood as task-bound authorization and
