...
containing products, columns, mandatory row scopes, a catalog digest, expiry,
and cumulative query, result-row, and database-time budgets.  An approval
...
\begin{enumerate}
  ...
  \item every relation reference is an unqualified logical product in $P_G$;
  every referenced column is in $C_G$; and stars or constructs that can expand
  the approved projection are rejected;
  \item every function, aggregate, and operator is in the catalog allowlist,
  and system catalogs, schema-qualified agent objects, and dangerous
...
\begin{enumerate}
  ...
  \item Check active state, grant/TTL, catalog digest, and schema readiness.
  ...
  \item Atomically settle actual use and result, or conservatively mark the
  request \code{INDETERMINATE}; append durable evidence from which a terminal
...
context.  Result collection is also capped in the connector.  A successful
result and settlement are persisted together where possible; transient
settlement failure makes readiness fail and enters an idempotent retry loop.
...
reconstruct whether the connector crossed the read boundary.

\subsection{Receipts and Audit}

A query receipt binds task, query and request identifiers; manifest, grant, and
catalog digests; request digest and canonical SQL fingerprint; policy decision;
budget before, reservation, charge, and budget after; row count and database
time; result hash; terminal status/error; timestamps; audit sequence and previous
audit hash; and a gateway key identifier.  The gateway signs canonical,
domain-separated receipt bytes with Ed25519.  The receipt contains result
metadata and a digest, not raw rows.
...
history.  Likewise, a query-receipt signature proves that the configured gateway
key signed the record, not that Business PG independently attested execution.

\subsection{Catalog and Schema Drift}

The signed authorization includes both catalog version and SHA-256 digest.
Every catalog product also gives the expected reporting-view column names,
order, and PostgreSQL types.  At startup and readiness checks, the connector
compares those expectations with \code{information\_schema.columns}.  Any
missing, extra, reordered, or mistyped column makes readiness fail and blocks
the execution path.  Catalog upgrades therefore require draining or replacing
...
The TLA+ specification~\cite{lamport2002specifying} is a finite abstraction of
one task.  It instantiates finite sets of request identifiers, receipt
identifiers, relations, columns, and scopes, together with a scalar budget.
Its task state is
...
terminal states \code{SETTLED}, \code{RELEASED}, or \code{INDETERMINATE}.
State variables record approval and catalog-validity flags, a monotonically
narrowed grant history, processed receipt identifiers and their effects,
scalar used/reserved budget, per-request grant and validity snapshots, request
state, and an abstract execution count.  Actions cover grant narrowing, a fresh
approval, approval replay, invalid approval rejection, reservation, duplicate
request attempts, connector/result/settlement phases, a definite pre-connector
release, three crash outcomes, revocation, expiry, rejection, completion, and
catalog drift.

The model deliberately matches the fixed boundary of one gateway and no more
than one in-flight query for the modeled task.  It does not model task
interleavings, manifest bytes, cryptography, SQL syntax, or the catalog/schema
attestation procedure.  Instead, grant containment and approval/catalog checks
are abstract predicates captured when a request starts.  In particular, the
model permits an already-reserved request to finish after a later terminal
transition; its terminal invariant forbids \emph{new} requests.  Parser,
signature, persistence, and schema-attestation behavior require separate tests.

\subsection{Safety Properties}

The configuration checks the following named invariants:
...
adds and settles one unit per request.  The implementation independently uses
the same reserve/settle discipline for queries, rows, and database milliseconds,
but TLC does not prove that vector lifting.
...
encrypted result bytes using AES-256-GCM, a plaintext result SHA-256 digest, and
append-only grant/audit rows protected from ordinary updates and deletes by
triggers.  A single audit-head row provides a total order.  These database
...
or signing.  OA receipts are Ed25519-signed and verified by key identifier.
For any terminal query, the gateway can separately construct and sign a receipt
binding task, query and request identities; manifest, grant, request, and catalog digests;
policy decision; before/reserved/charged/after budget evidence; outcome and
result hash; and the corresponding audit-chain position.  Its dedicated key is
...
The four baselines are (B1) direct PostgreSQL, (B2) native reporting
View/RLS, (B3) an AST-only gateway without approval, control ledger, encrypted
result persistence, or signed receipt, and (B4) full \system.  The workload uses
TPC-H and TPC-DS at SF1 and SF10.  Each configuration runs at concurrency
$1,8,32$ after warmup; every concurrent worker uses a distinct task so the
experiment does not violate per-task serialization.  Each cell requires at
least 30 measured repetitions.

The driver records throughput, p50/p95/p99 end-to-end latency, gateway and
database CPU, peak resident memory, Control PG transaction counts/latency,
receipt bytes, and available baseline-specific component times.  For the full
...
storage, kernel, Docker, Go, PostgreSQL, image digests, dataset checksums, commit,
warmup rule, seed, and exact command.  Baselines use the same query texts, data,
client host, connection policy, and result consumption.  Raw JSON or CSV is the
sole source for reported tables and figures.
...
\begin{itemize}\raggedright
  ...
  tests;
  \item \code{make formal}: TLC with archived configuration and log;
...
$used+reserved\leq limit$ and one in-flight query per task & \code{internal/control/budget.go}, task/budget row locks, TLA+ \code{BudgetSafety} and \code{SingleInFlight} & Store concurrency tests; finite TLC status is generated separately and is not a general proof. \\
...
\begin{itemize}
  ...
  relative to evidence retained outside the mutable history.  The prototype has
  no WORM replica, transparency log, trusted timestamp, or external chain-head
  anchor.
  ...
  manager defeats the corresponding guarantee.  Schema attestation checks shape,
  not the semantic correctness of a view definition.  Its V1 type check uses
  generic \code{information\_schema.data\_type}; precision, scale, length,
  domains, and array element types are not attested.
  ...
  outbox, key rotation/KMS, backup/restore, retention, rate limits, and
